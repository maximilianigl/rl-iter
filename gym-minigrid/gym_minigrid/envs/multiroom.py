...
class MultiRoomEnv(MiniGridEnv):
    ...
    def _gen_grid(self, width, height):
        ...
        if self.random_start:
            room_id =  self._rand_int(0, numRooms)
        else:
            room_id = 0
        # Randomize the starting agent position and direction
        self.place_agent(roomList[room_id].top, roomList[room_id].size)
...
# v0 is with starting position in the furthest room
# v1 is a modification with the starting position anywhere in the grid. Makes training a bit easier, e.g. for debugging
# or faster iteration.
# Note that the grid sizes aren't constant across different environments!
register(
...
# For evaluation: Fixed number of rooms instead of random, all with grid size 13 like the N4r
register(
